...
echo ""
last_version=$(curl -Ls "https://api.github.com/repos/kahpoh19/X-Panel/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
# 获取 x-ui 版本
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        echo ""
        sleep 2
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/kahpoh19/X-Panel/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/kahpoh19/X-Panel/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz"
        echo ""
        echo -e "--------------------------------------------"
    ...
    wget -O /usr/bin/x-ui-temp https://raw.githubusercontent.com/kahpoh19/X-Panel/main/x-ui.sh
...
echo -e "----------------------------------------------"
echo ""
echo -e "${green}〔X-Panel面板〕项目地址：${yellow}https://github.com/kahpoh19/X-Panel${plain}" 
echo ""
